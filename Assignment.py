# -*- coding: utf-8 -*-
"""
Created on Wed Dec 10 14:39:24 2023
...
def load_dataframe_country_col(filename):
    '''
...
def year_wise_data(data,s_y,e_y,skip_years_freq):
    '''
...
def filter_column_value(data,column,values):
    '''
...
def bar_plot_for_country(data,indicator_variable):
    df_sample = data.copy()
...
df_year_column, df_country_column = \
    load_dataframe_country_col('world_bank_climate.xlsx')

#collect data from 1970 to 2020 with 5 interval timeframe
df_year_sample = year_wise_data(df_year_column,1970,2020,5)

#10 countires for analysis regarding indicator occurance
countries_to_check = \
    df_year_sample['Country Name'].value_counts().index.tolist()[15:25]
df_year_sample['Country Name'].value_counts()

#filter dataframe for countries required
df_year_sample_country  = filter_column_value(
    df_year_sample,
    'Country Name',
    countries_to_check)
...
df_year_sam_co2 = filter_column_value(
    df_year_sample,'Indicator Name',
    ['Agricultural land (sq. km)'])
print(df_year_sam_co2.describe())

df_year_sam_co2_cont  = filter_column_value(
    df_year_sam_co2,
    'Country Name',
    countries_to_check)
bar_plot_for_country(df_year_sam_co2_cont,'Agricultural land (sq. km)')

df_year_col_mort = filter_column_value(
    df_year_sample,
    'Indicator Name',
    [
...
bar_plot_for_country(
    df_year_col_mort,
    'Electricity production from renewable sources, excluding hydroelectric (% of total)')


df_year_high_income= filter_column_value(
    df_year_sample,
    'Country Name',
    ['High income'])
...
df_year_col_energy= filter_column_value(
    df_year_sample,
    'Indicator Name',
    ['Electricity production from coal sources (% of total)'])
df_year_col_energy  = filter_column_value(
...
df_year_col_urban= filter_column_value(
    df_year_sample,
    'Indicator Name',
...
df_year_co2= filter_column_value(
    df_year_sample,
    'Indicator Name',
    ['CO2 emissions from gaseous fuel consumption (kt)'])
df_year_co2  = filter_column_value(
...
df_year_oecd = filter_column_value(
    df_year_sample,
    'Country Name',
    ['OECD members'])
...
df_year_col_post= filter_column_value(
    df_year_sample, 'Country Name', ['Post-demographic dividend'])
